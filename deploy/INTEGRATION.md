...
| 服务 | 本机入口 | 版本 |
| --- | --- | --- |
| MediaDock | http://127.0.0.1:8080 | 当前工作区，含本次兼容修复 |
...
| MCP | initialize、tools/list、media_job_status 正常；六个工具可发现 |
...
| WSL fork 网盘 → Local 复制 | **失败**：选取的 6,964,263 字节文件下载返回 HTTP 412，零字节落盘不能算成功 |
...
这说明转存可用，NAS 原下载路径也可用；**不表示 WSL fork 已完成网盘下载，更不表示 MediaDock 已接入网盘获取**。
...
下一步应先定位 WSL 下载 412，再为 MediaDock 添加 OpenList 获取适配器及多阶段持久化；现有 `media_acquire` 仍仅接入 BT 下载器。
